...
The proposed software product (Online Beauty academy) is an online education system. The system will be used for online-education, to study online-courses, to view and download differnt lectures, conducting online quizzes, course registration, exam reservation, managing results. The system must be right protected.\\
This product let persons who have interest to study beauty , who want to make them self more beautiful to do it. It will ofer also other different online courses. There are  beauty-course , beauty-instructor course and other future courses.
...
Online management system is effective, reduce time and cost in courses and exam management process.
  	...
  	The main goal of this project is to make part of this big system. It will include use cases for user, participant , lecturer and administrator. 
  Now I want to describe the main goals that i have planned to do during this semester. \\The first goal is to make user possible to register himself in this application. The system has to be able to recognize  different roles of user. The system have to be well secure , for this purpose i will use the security staffs that offers spring boot framework. Then user can easily log in in the system and see all available courses in course catalogue. \\
  For administrator it is necessary to implement managing tools, so that administrator can manipulate courses, like add a new course, delete, update, etc. I will implement also the administrator tools for manipulate all users.\\
  The other goal is to make possible user to register for course that he likes . And one more goal is to make possible participant to order an exam. 
...
First you have to make steps that are described here \url{ https://www.postgresql.org/}\\ You have to create a new data base , create name for it.
...
\end{figure}
 ...
 Figure 17 shows the application.properties file, that you have to change by putting you username , password for posgresQL.\\
...
I can say that we have achieved my plan  , but not completely. I have created the necessary database based on system analysis and implementation. I  have made the system that works, the connection with database ,connection between frontend and backend. And I have realised the features like:\\
-login\\
-registration\\
...
The first thing I want to underline here that the project management have to be done better as I've done this time. I have made big Objective for this project without research how much time i need to improve it.\\
At first I had to spend  more time for analysis what it is really possible to do in this time. For this project I wanted to use the last technologies. So I had to calculate more time, because one of technologies was new for me. \\
This project i have made from null to actual state , so it was very important the software engineering steps to do well, using enough time.\\
In other hand making big Objective i have achieve also gut results. The biggest part of project is achieved.
